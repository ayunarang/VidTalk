...
const MeetingsDashboard = () => {
  ...
  const exportPDF = async () => {
    const pdf = new jsPDF();
    const canvas = await html2canvas(document.querySelector('#notesContent'), { backgroundColor: null });
  ...
  return (
    <>
      <div className={styles.container}>
        <div className={styles.headingBox}>
          ...
          <div className={styles.rightHeader}>
            <button onClick={() => setisScheduleMeeting(true)} className={styles.btn}>Schedule Meeting</button>
            <button onClick={() => router.push("/")} className={styles.btn}>Create Meeting</button>
            ...
            return (
              <div key={meeting._id} className={styles.meetingCard}>
                ...
                <div className={styles.meetingInfo}>
                  ...
                  <div><p className={styles.name}>Hosted by {meeting?.host?.name}</p></div>
                </div>
      ...
      <div className={`absolute right-0 top-0 bottom-0 w-96 h-full bg-[#1e1e1e] shadow-lg z-50 transition-transform duration-700 ease-in-out transform ${openComponent === "chat" ? "translate-x-0" : "translate-x-full"}`} style={{ overflowY: 'auto' }}>
        {openComponent === "chat" && <Chat isSavedChat={true} customChat={customChat} />}
      </div>
      <div className={`absolute right-0 top-0 bottom-0 w-96 h-full bg-[#1e1e1e] shadow-lg z-50 transition-transform duration-700 ease-in-out transform ${openComponent === "notes" ? "translate-x-0" : "translate-x-full"}`} style={{ overflowY: 'auto' }}>
        {openComponent === "notes" && (
          note || note.length > 0 ? (
      ...
      <div className={`absolute right-0 top-0 bottom-0 w-96 h-full bg-[#1e1e1e] shadow-lg z-50 transition-transform duration-700 ease-in-out transform ${openComponent === "participants" ? "translate-x-0" : "translate-x-full"}`} style={{ overflowY: 'auto' }}>
        {openComponent === "participants" && MeetingParticipants && (
          <div className={styles.participantsContainer}>
            <h2 className='text-2xl font-bold mb-3'>Meeting Participants</h2>
            <hr className="border-neutral-700 rounded mb-4 mt-4" />
            <ul>
              {MeetingParticipants.map(participant => (
                <li key={participant._id} className={styles.participantItem}>
                  <div className={styles.participantDetails}>
                    <img src={participant.avatar} alt={participant.name} className={styles.avatar} />
